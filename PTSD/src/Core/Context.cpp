...
Context::Context(unsigned int width, unsigned int height)
    ...
    m_Window =
        SDL_CreateWindow(TITLE, WINDOW_POS_X, WINDOW_POS_Y, m_WindowWidth,
...
//     return s_Instance;
// }
std::shared_ptr<Context> Context::GetInstance(unsigned int width,
                                              unsigned int height) {
    if (s_Instance == nullptr) {
        s_Instance = std::make_shared<Context>(width, height);
    }
    return s_Instance;
}
